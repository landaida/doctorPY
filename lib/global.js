savePaciente = function(e, template) {
  ...
  var onlyRequired = _.omit(paciente,'telefono2', 'telefono3', 'otrosDatos', 'email', 'aseguradora');
  ...
  if (!this._id) {
    ...
    Meteor.call('pacienteUpdate', this._id, paciente, function(error, result) {
      ...
      }else{//Si no hay foto decir al usuario que se guardo con exito
        FlashMessages.sendSuccess(MENSAJE_SAVE_SUCCESS);
      }
    })
    /*************************************************/
    /***************FIN UPDATE PACIENTES**************/
    /*************************************************/
  }
...
imc = function(peso, altura, isHistory){
  var alturaMetros = 0, imc = 0, mosteller = 0, boyd = 0, haycock = 0, punto = altura.toString().indexOf('.'), comma = altura.toString().indexOf(',');
  if(peso && altura){
  ...
  if(!isHistory){
    ...
    Session.set('imc-h', imc);
    Session.set('mosteller-h', mosteller);
    Session.set('boyd-h', boyd);
    Session.set('haycock-h', haycock);
  }
