...
Meteor.methods({
  pacienteInsert: function(pacienteAttributes) {
    ...
    if (errors.dni || errors.nombre)
      throw new Meteor.Error('invalid-paciente', "Usted debe completar el DNI, Nombre, Edad y sexo de su paciente");

    //Evita duplicidad de urls
    var pacienteWithSameLink = Pacientes.findOne({url: pacienteAttributes.url});
