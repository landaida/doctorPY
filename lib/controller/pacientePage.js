PacientePageController = RouteController.extend({
  ...
  tiposDosis: function(){
    var tipos =  Tipos.find({tipo: TIPOS_DOSIS}, {sort:{_id:1}}).fetch();
    Session.set('tiposDosis', tipos);
  },
  tiposFrecuencia: function(){
    var tipos =  Tipos.find({tipo: TIPOS_FRECUENCIA}, {sort:{_id:1}}).fetch();
    Session.set('tiposFrecuencia', tipos);
  },
  tiposDuracion: function(){
    var tipos =  Tipos.find({tipo: TIPOS_DURACION}, {sort:{_id:1}}).fetch();
